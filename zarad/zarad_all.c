...
void singl_vector(std::vector<sf::Vector3f> ch, Vector2f beg) {
	...
	*/
		...
		if (ch[1].z == 0) {
			second.x = beg.x - ch[1].x;
			second.y = beg.y - ch[1].y;
		}
		else {
			second.x = ch[1].x - beg.x;
			second.y = ch[1].y - beg.y;
		...
		fist1 = (fist / len(fist)) / f / f ;
		second1 = (second / len(second)) /  s / s ;

		result.x =  second1.x + fist1.x;
		result.y =  second1.y + fist1.y;
		...
		for (int i = 2; i < quantaty; i++) {
			...
			float  x = len(second);
			second1 = (second / len(second)) / x / x;

			result = result + second1;
